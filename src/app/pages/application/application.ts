...
export class Application implements OnInit {
  ...
  availableStatuses: ApplicationStatus[] = ['En ligne', 'Alerte', 'Arrêtée', 'Maintenance'];
  ...
  form = {
    ...
    serveur: '',
    version: '',
    statut: 'En ligne' as ApplicationStatus,
    description: '',
    responsable: '',
    environnement: 'Production',
    technologie: '',
  ...
  addApplication(): void {
    ...
    const technologies = this.form.technologie
      .split(',')
      .map(item => item.trim())
    ...
    const newApplication: ApplicationItem = {
      ...
      serveur: this.form.serveur.trim(),
      version: this.form.version.trim(),
      statut: this.form.statut,
      heure: `${hh}:${mm}`,
      description: this.form.description.trim(),
      responsable: this.form.responsable.trim(),
      environnement: this.form.environnement.trim(),
      technologie: technologies.length ? technologies : ['Application métier'],
      disponibilite: Number(this.form.disponibilite) || 95,
      latence: Number(this.form.latence) || 120,
      charge: Number(this.form.charge) || 35,
      incidents: Number(this.form.incidents) || 0,
      dernierDeploiement: this.form.dernierDeploiement.trim() || 'Aujourd’hui',
      utilisateurs: this.form.utilisateurs.trim() || '0'
    };
  ...
  updateStatus(newStatus: ApplicationStatus): void {
    this.selectedApplication.statut = newStatus;
